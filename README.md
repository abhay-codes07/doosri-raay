...
|---|---|---|
| Frontend | `<NAME>` | React PWA, bilingual auth, parent tile with check-in / Puchho / covert SOS, guardian dashboard and task cards, case intake and detail with source lines, Settings, `/demo`, `/try`, service worker, Web Push |
| Backend and model | `<NAME>` | API handlers, classifier worker (forced tool use), Strands recovery agent with deterministic fallback, narrative re-check, rules as data, sources verification, Polly cache, eval set and harness, 98 tests |
