...
- `ladder-task` payload: `{kind, rung?, assigneeRole, circleId, parentSub?, caseId?, reason?, wait, taskToken?, executionArn?, timeouts?, attempt?, reminder?, escalation?}`. The four Ladder rungs pass `executionArn` (`$$.Execution.Id`) so the backend can store the running ladder ARN and `StopExecution` it on check-in, and `timeouts` (the execution's `{rung}` object) so the task item can carry its own expiry. `Rung4Emergency` has a `States.ALL` Catch into `MarkEscalated`, so `ladderState=escalated` is written even when the 112 task Lambda fails. `wait:true` states use `.waitForTaskToken`; `wait:false` states (rung 4 emergency, 1930 / NCRP escalations, confirm-fields reminder) are plain invokes and the task is informational.
- `ladder-status` payload: `{ladderState: watching|escalated|ok, closeOpenTasks: bool, circleId, parentSub, reason}`.
- `watch-check` payload `{circleId, parentSub, startedAt, sinceTs, deadline, executionArn}` -> `{checkedIn: bool, holidayMode: bool}` (a CHECKIN with `ts >= sinceTs` counts; `startedAt` is kept for older executions; `executionArn` is `$$.Execution.Id`, so the backend can treat a watch that is no longer the parent's active one as checked in and let it end quietly).
