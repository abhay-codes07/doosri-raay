...
Placeholders in angle brackets are for the team to fill before pasting. Covers both tracks (Ship It and Build It).
"""
...
LIVE = "https://main.d2inambt66a14p.amplifyapp.com"

# ----------------------------------------------------------------------------------------------
# Answers. Blank line = new paragraph; "- " prefix = bullet; "## " prefix = sub-heading inside an
# answer. <ANGLE> tokens are placeholders to fill before pasting.
# ----------------------------------------------------------------------------------------------

TEAM_FIELDS = [
    ("Team leader's WeMakeDevs username", True, "<ABHAY_WEMAKEDEVS_USERNAME>  (copy it from wemakedevs.org/home)"),
    ("Second team member's WeMakeDevs username", False, "<RAJAT_WEMAKEDEVS_USERNAME>"),
    ("Third / fourth team member's WeMakeDevs username", False, "leave blank (two-person team)"),
...
PROJECT_FIELDS = [
    ...
    ("Track you are submitting for", True,
     "Both. Ship It (the stack is deployed on AWS and live) and Build It (the same code uses the AWS open-source stack: "
     "Strands Agents SDK, AWS SAM, LocalStack and Cedar). If the form allows one choice, choose Ship It."),
    ("GitHub link to project", True,
     f"{REPO}\n\nPublic, MIT-licensed, 100+ commits between 18 and 20 September with a judge-facing README, "
     "docs/ (architecture, API and state-machine contracts, prior art, AI-tools disclosure, demo shot list, research), "
     "230 backend tests and a 70-item evaluation set."),
    ("Deployed link to project", False,
     f"{LIVE}/try\n\nNo sign-up needed. /try signs you into a seeded judge circle: the parent's tile on the left, the "
     "guardian's dashboard on the right, a judge FAQ with live timers and a Reset button. Three judge circles exist "
     "(judge1, judge2, judge3 @demo.doosriraay.in) so judges never interfere with each other. Password for the "
     "optional full sign-in path at /signin: <JUDGE_PASSWORD>."),
    ("YouTube video demo link", True,
     "<YOUTUBE_URL>  (unlisted, 3:00). 0:00 the problem and the idea; 0:18 the parent tile; 0:33 Watch to Ladder in "
     "Step Functions; 1:03 the checker; 1:15 Puchho with Polly; 1:27 the recovery case; 2:05 every AWS service in "
     "the console; 2:29 sources verified; 2:49 what we learned."),
]

WHAT_IT_DOES = """## The problem
...
HOW_AWS = """## Ship It: the AWS services
...
CONTRIB_LEADER = """Abhay Singh (team leader): research, architecture, backend, infrastructure.
...
CONTRIB_SECOND = """Rajat Nagda: frontend, deployment and operations, demo.
...
CHECKLIST = """- The repository is public.
...
SECTIONS = [
    ...
    ("How did you use AWS in your project? (required)", [("Build It: the AWS open-source stack. Ship It: the AWS services.", True, HOW_AWS)]),
    ...
    ("Before pasting", [("Checklist", False, CHECKLIST)]),
]
...
def answer_flowables(text: str, st: dict):
    out = []
    for block in text.strip().split("\n\n"):
        if block.startswith("## "):
            out.append(Paragraph(esc(block[3:]), st["h3"]))
            continue
        lines = block.split("\n")
        if all(line.startswith("- ") for line in lines):
            for line in lines:
                out.append(Paragraph("\u2022 " + esc(line[2:]), st["bullet"]))
        else:
...
def build_pdf() -> None:
    ...
    st = {
        "title": ParagraphStyle("t", fontName=bold_font, fontSize=18, leading=22, spaceAfter=3 * mm),
        "sub": ParagraphStyle("s", fontName=body_font, fontSize=9.5, leading=13, textColor=colors.HexColor("#555555"), spaceAfter=5 * mm),
        "h1": ParagraphStyle("h1", fontName=bold_font, fontSize=13.5, leading=17, spaceBefore=6 * mm, spaceAfter=2.5 * mm, textColor=colors.HexColor("#1f3a5f")),
        "q": ParagraphStyle("q", fontName=bold_font, fontSize=10.5, leading=14, spaceBefore=3 * mm, spaceAfter=1.5 * mm),
        "h3": ParagraphStyle("h3", fontName=bold_font, fontSize=10, leading=13, spaceBefore=2 * mm, spaceAfter=1 * mm, textColor=colors.HexColor("#333333")),
        "body": ParagraphStyle("b", fontName=body_font, fontSize=9.6, leading=13.2, alignment=TA_LEFT),
        "bullet": ParagraphStyle("bl", fontName=body_font, fontSize=9.6, leading=13.2, leftIndent=5 * mm, firstLineIndent=-3 * mm),
    }
    ...
    story = [
        Paragraph("Doosri Raay: First Commit submission form answers", st["title"]),
        Paragraph("WeMakeDevs x AWS Builder Center, First Commit (17-20 Sep 2026). Tracks: Ship It and Build It. "
                  f"Prepared 20 Sep 2026. Text in &lt;ANGLE BRACKETS&gt; is a placeholder to fill before pasting. "
                  f"Repository: {REPO} &nbsp;|&nbsp; Live: {LIVE}/try", st["sub"]),
    ]
    for title, fields in SECTIONS:
        ...
        if title in ("Team", "Project"):
            rows = [[Paragraph("<b>Field</b>", st["body"]), Paragraph("<b>Answer</b>", st["body"])]]
            for q, req, a in fields:
                rows.append([Paragraph(esc(q) + (" *" if req else ""), st["body"]),
                             Paragraph(esc(a).replace("\n", "<br/>"), st["body"])])
            ...
            for q, req, a in fields:
                story.append(Paragraph(esc(q) + (" *" if req else ""), st["q"]))
                story.extend(answer_flowables(a, st))
    doc.build(story)


def build_md() -> None:
    lines = ["# Doosri Raay: First Commit submission form answers", "",
             "Copy-paste source for https://wemakedevs.org/aws/first-commit/submit (tracks: Ship It and Build It). "
             "Text in `<ANGLE BRACKETS>` must be filled before pasting. PDF twin: `docs/Submission.pdf`.", ""]
    for title, fields in SECTIONS:
        lines += [f"## {title}", ""]
        for q, req, a in fields:
            body = a.strip().replace("\n## ", "\n#### ")
            lines += [f"### {q}{' *' if req else ''}", "", body, ""]
