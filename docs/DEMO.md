...
| 7 | 2:45–3:00 | Close | Eval table from `eval/results.md` (summary row). Prior-art slide: Kavach, Rakshak, SwarVed and the four gaps. Roadmap: bank-counter copilot, Khyaal/Emoha distribution. | "A seventy-item Hindi and English test set with benign controls, run through an evaluation harness against Bedrock; the numbers are in the README. What we learned: build the outsider." | End card: repo URL, live URL. |
